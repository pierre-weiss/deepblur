...
class UNet(nn.Module):
    def __init__(self, num_input_channels=3, num_output_channels=3, feature_scale=4, more_layers=0, concat_x=False,
                 up_sample_mode='deconv', pad='zero', norm_layer=nn.InstanceNorm2d, need_sigmoid=True, need_bias=True):
        """
        Parameters
        ----------
        num_input_channels
        ...
        more_layers
        concat_x
        up_sample_mode in ['deconv', 'nearest', 'bilinear']
        ...
        if self.more_layers > 0:
            ...
            self.more_ups = [unetUp(filters[4], up_sample_mode, need_bias, pad, same_num_filt=True) for i in
                             range(self.more_layers)]

            self.more_downs = ListModule(*self.more_downs)
            self.more_ups = ListModule(*self.more_ups)

        self.up4 = unetUp(filters[3], up_sample_mode, need_bias, pad)
        self.up3 = unetUp(filters[2], up_sample_mode, need_bias, pad)
        self.up2 = unetUp(filters[1], up_sample_mode, need_bias, pad)
        self.up1 = unetUp(filters[0], up_sample_mode, need_bias, pad)
...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, norm_layer, need_bias, pad):
        ...
        if norm_layer is not None:
            self.conv1 = nn.Sequential(conv(in_size, out_size, 3, bias=need_bias, pad=pad), norm_layer(out_size),
                                      nn.ReLU(),)
            self.conv2 = nn.Sequential(conv(out_size, out_size, 3, bias=need_bias, pad=pad), norm_layer(out_size),
                                      nn.ReLU(),)
        else:
...
class unetUp(nn.Module):
    def __init__(self, out_size, up_sample_mode, need_bias, pad, same_num_filt=False):
        super(unetUp, self).__init__()

        num_filt = out_size if same_num_filt else out_size * 2
        if up_sample_mode == 'deconv':
        ...
        elif up_sample_mode =='bilinear' or up_sample_mode =='nearest':
            self.up = nn.Sequential(nn.Upsample(scale_factor=2, mode=up_sample_mode), conv(num_filt, out_size, 3,
